...
namespace plume::bench {
    ...
    // Renders a full-screen single-color triangle and reads the framebuffer back.
    // Exercises the combined draw path (bind pipeline + vertex buffer + draw + submit)
    // and verifies real rendered output.
    inline Workload createFrameMacroWorkload(RenderDevice *device, RenderCommandQueue *queue, RenderShaderFormat fmt) {
        struct Fixture {
            RenderDevice *device;
        ...
        Workload w;
        w.name = "frame_macro_render_readback";
